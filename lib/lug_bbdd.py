...
def ListarCamposTable(Credenciales, NombreTabla):
    '''Dada una base de datos definida en Credenciales, y una tabla definida en un esquema de esa base de datos.
    Devuelvo la lista de los campos de la tabla con su nombre, el tipo y el tamaño de loas variables que son string.
    La base de datos que consulto es Maria DB'''
    sql = f"SELECT COLUMN_NAME AS NOMBRE, DATA_TYPE AS TIPO, CHARACTER_MAXIMUM_LENGTH AS LONGITUD FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '{NombreTabla}'"
    #print(sql)
    Datos = []
    resultado, Datos = ObtenerDatos_MySQL_enJSON(Credenciales=Credenciales, SQL=sql)
    return Datos

def InsertarRegistro (Credenciales, Tabla, Datos):
    ''' Inserta Datos en una tabladefinitida en Credenciales
    Se asume que los atributos de Datos definidos coinciden los campos definidos en la Tabla
    Devuelvo si se ha ejecutado la conseulta u los errores si los hubiera
    '''
    Aux = False
    Errores = {}
    if isinstance(Datos, dict):
        # Compongo la sentencia SQL
        campos = ', '.join(Datos.keys())
        valores = ', '.join(['%s'] * len(Datos))
        query = f"INSERT INTO {Tabla} ({campos}) VALUES ({valores})"
        # Trabajo los datos para la SQL
        Data = {}
        for k,v in Datos.items():
            if isinstance(v, dict) or isinstance(v, list):
                Data[k] = json.dumps(v, ensure_ascii=False) #Para insertar los caracteres URF-8
            else:
                Data[k] = v
        DatosaInsertar = tuple(Data.values())
        try:
            conexion = mysql.connector.connect(
                ...
                database=Credenciales[4]
            )
            cursor = conexion.cursor()
            #Debo pasar una tupla de datos
            cursor.execute(query, DatosaInsertar)
            # Confirmar y cerrar la conexión
            conexion.commit()
        except mysql.connector.IntegrityError as integrity_error:
            Error = f"Error de integridad en la conexión o consulta: {integrity_error}"
            Errores['error'] = integrity_error
            Errores['sql'] = query
            Errores['datos'] = DatosaInsertar
            logging.error(Error)
        except mysql.connector.Error as error:
            Error = f"Error en la conexión o consulta: {error}"
            Errores['error'] = error
            Errores['sql'] = query
            Errores['datos'] = DatosaInsertar
            logging.error(Error)            
        except ValueError as value_error:
            Error = f"Error de valor: {value_error}"
            Errores['error'] = value_error
            Errores['sql'] = query
            Errores['datos'] = DatosaInsertar
            logging.error(Error)
        except Exception as general_error:
            Error = f"Error general: {general_error}"
            Errores['error'] = general_error
            Errores['sql'] = query
            Errores['datos'] = DatosaInsertar
            logging.error(Error)
        ...
        Error = 'Datos como parámetros debe de ser un diccionario'
        Errores['error'] = Error
        Errores['sql'] = query
        Errores['datos'] = DatosaInsertar
        logging.error(Error)
...
def ObtenerDatos_MySQL_enJSON(Credenciales, SQL):
    """
        Dada una consulta select obtengo el resultado de la consulta
        en el servidor definido con las credenciales y devuelvo un objeto json
        Si hay error devuelvo un objeto vacío
        Devuelvo una lista con un true o false para saber si he ejecutado correctamente la consulta
        y otra lista con lo datos en formato json
    """
    # Hemos validado que existentes las credenciales de acceso a MySQL
    host = Credenciales[0]
    port = Credenciales[1]
    user = Credenciales[2]
    passwd = Credenciales[3]
    db = Credenciales[4]

    EstadoConsulta = False    
    Resultado = []
    Aux = [] # Lista de Errores para mostrar y guardar
    try:
        con = pymysql.connect(host=host,user=user,password=passwd,db=db, charset='utf8mb4', cursorclass=pymysql.cursors.DictCursor, local_infile=True)
        cursor = con.cursor()
        cursor.execute(SQL)        
        #Obtengo las cabeceras de la consulta
        cabeceras=[x[0] for x in cursor.description]
        ...
        logging.error(ErrorDato)
        Aux.append(ErrorDato)
    return EstadoConsulta, Aux

def EjecutarSQL_MySQL(Credenciales, SQL):
    """
        Ejecuta una sentencia SQL en MySQL
        Credenciales tiene los parámetros de conexión
        Devuelvo True o False dependiendo si se ha ejecutao o no la centencia SQL
        Credenciales es una lista con los parámetros básicos
        Sirve para insert, update, delete y create
    """    
    EstadoConsulta = False 
    ErrorDatos = {}
    ...
    try:
        ...
        logging.error(ErrorDatos)
    return EstadoConsulta, ErrorDatos
